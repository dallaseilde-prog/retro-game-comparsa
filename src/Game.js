...
export class Game {
  ...
  loadLevel(index) {
    ...
    if (!level) { this.setState('VICTORY'); return; }
  ...
  draw() {
    ...
    gradient.addColorStop(1, '#0c001a');
    this.ctx.fillStyle = gradient;
    this.ctx.fillRect(0, 0, this.width, this.height);
    this.ctx.fillStyle = '#100028';
    this.ctx.beginPath();
  ...
  collectCoffee() { this.score++; this.audio.playCollect(); this.updateHUD(); }
  winLevel() {
    this.audio.playWin();
    if (this.levelIndex + 1 < this.levelManager.levels.length) this.loadLevel(this.levelIndex + 1);
